...
def main():
    ...
    parser.add_argument('--data_path', type=str, default='../datasets/Yelp/')
    ...
    tensorboard_path = f'tensorboard/{args.dataset}/timestr'
    logger = get_logger(f'logs/{args.dataset}/{timestr}.log')
    logger.info(args)
    ...
    F1, acc = test_metrics
    logger.info('Tst F1={:.4f} ACC={:.4f}'
                .format(F1, acc))
    metrics_list.append(test_metrics)

    metrics = np.array(metrics_list)
    means = metrics.mean(axis=0)
    print(f'{args.dataset} Test Summary:')
    logger.info('Mean F1={:.4f}, ACC={:.4f}'.format(
        means[0], means[1]))
    logger.info("Done")
