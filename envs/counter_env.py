...
class TestEnv(object):
    def __init__(self, user_num, memory, memory_rate, all_nbr, all_nbr_iid, all_nbr_iid_rate, args):
        ...
        self.GT_user_emb = torch.from_numpy(np.load(f'../datasets/{args.dataset}/ground_truth_user_emb.npy'))
        self.GT_item_emb = torch.from_numpy(np.load(f'../datasets/{args.dataset}/ground_truth_item_emb.npy'))
    ...
    def step(self, users, action_id, action):
        ...
        item_embedding = self.GT_item_emb[action]
