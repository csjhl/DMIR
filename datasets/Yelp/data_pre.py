...
def load_and_save_yelp():
    if os.path.exists(saved_path + 'uid_map.npz'):
        return
    user_path = r'./yelp_academic_dataset_user.json'
    u2u_list = list()
    ...
    rating_path = r'./yelp_academic_dataset_review.json'
    u2i_list = list()
    ...
    np.savez(file=saved_path + 'u2ui.npz',
             u2i=u2i,
             u2u=u2u)
    np.savez(file=saved_path + 'uid_map.npz',
             uid_map=uid_map)

    print('saved at', 'u2ui.npz')


def filter_and_reid():

    save_path = saved_path + 'reid_u2ui.npz'
    if os.path.exists(save_path):
        return
    u2ui = np.load(saved_path + f'u2ui.npz')
...
def delete_isolated_user():
    ...
    if os.path.exists(save_path):
        tmp = np.load(saved_path + 'user_item_num.npy')
        return tmp[0], tmp[1]
    u2ub = np.load(saved_path + 'reid_u2ui.npz')
    uu_elist = u2ub['u2u']
    u2i = u2ub['u2i']
...
def gen_and_save_u2u_dict_and_split(num_user, num_item):
    if os.path.exists(saved_path + f'{valid_scale}u2u_split_dicts.pkl'):
        return
...
def get_Z_bin(item_num):
    if os.path.exists(saved_path + f'{time_interval_size_day}day_{valid_scale}valid_scale_Z_bin.npz'):
        return
...
def all_get_Z(user, seq, ts, nbrs, train_Z_bin, all_Z_bin, time_interval_size, user_train, user_valid, min_time):
    Z = np.zeros(seq_maxlen, dtype=np.float)
    Z_d = np.zeros(seq_maxlen, dtype=np.float)
...
def get_self_nbr_item_set(user_num):
    if os.path.exists(saved_path + f'{valid_scale}self_nbr_item_set.npy'):
        return
    u2u_dict, user_train, user_valid,eval_users, item_nums_per_user, train_mat = \
        load_pkl(saved_path + f'{valid_scale}u2u_split_dicts.pkl')
...
def gen_and_save_all_user_batches(user_num, item_num):
    ...
    for user in tqdm(range(1, user_num)):
        ...
        if valid_Z[0] != -1:
            all_valid[user] = np.concatenate((user_valid[user], np.expand_dims(valid_Z, -1)), axis=-1)#valid_item_num*4 (item, ts, rate, Z)
...
def preprocess_yelp():
    load_and_save_yelp()
    filter_and_reid()
    num_user, num_item = delete_isolated_user()
    num_user = num_user + 1
    num_item = num_item + 1
    ...
    get_Z_bin(num_item)
    get_self_nbr_item_set(num_user)
